...
=========
Расписание:
- Отчёт по часу:  проверка каждые 5 мин в окна 9:35-10:05, 12:35-13:05, 14:55-15:25, 19:45-20:15
                  отправляет сразу как получит
- День/Неделя/Месяц: письмо приходит в 6-7 утра, бот держит и отправляет в 6:00
  • День     — каждый день
...
def get_hourly_window(hour, minute):
    ...
    windows = [(9,35,10,5),(12,35,13,5),(14,55,15,25),(19,45,20,15)]
    t = hour*60 + minute
...
async def main():
    ...
    # Кэш для хранения отчётов полученных до 6:00
    ...
    while True:
        ...
        # Шаг 1: проверяем почту с 3:00 до 5:59 — кэшируем, не отправляем
        if 3 <= hour < 6 and minute < 5:
            cache_key = f'cached_{today}'
            if cache_key not in cached and not already_sent(f'day_{today}'):
                log.info('Проверяем почту (кэш до 6:00)...')
                result = (
        ...
        # Шаг 2: в 6:00 отправляем всё что накопилось
        if hour == 6 and minute < 5:
            ...
            if cache_key not in cached and not already_sent(day_key):
                log.info('6:00 — проверяем почту напрямую...')
                result = (
                    fetch_attachment('ежедневный отчет') or
                    fetch_attachment('отчет по продажам') or
                    fetch_attachment('продажам за')
            ...
            if cache_key in cached and not already_sent(day_key):
                ...
                log.info(f'6:00 — отправляем отчёты: {fn}')
